...
<section class="container-fluid">
    <h2>
        <span class="text-capitalize">{!! $crud->getHeading() ?? $crud->entity_name_plural
            !!}</span>
        <small>{!! $crud->getSubheading() ?? trans('backpack::crud.edit').' '.$crud->entity_name_plural !!}.</small>

        @if(isset($crud->backLink))
        @else
        @if ($crud->hasAccess('list'))
        <small><a href="{{ isset($crud->backLink) && $crud->backLink ? url()->previous() : url($crud->route) }}"
